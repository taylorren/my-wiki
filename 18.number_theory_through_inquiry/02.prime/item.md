...
我们可以知道$(a-b)\mid (a^n-b^n)$，令$a=2^r, b=-1, m=s$，得到：$(2^r+1)|(2^{rs}+1) \implies (2^r+1)\mid 2^k+1$。与题设矛盾。所以，$n$必然是2的幂的形式。QED。

我们已经知道，形如$2^p-1$这样的素数称为梅森素数。而形如$2^{2^k}+1$这样的素数被称为费马素数。
