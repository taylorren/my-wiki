...
**定义3**：假定$a$和$d$是两个整数，我们定义$d$**整除**$a$，记做$d|a$，当且仅当存在一个整数$k$，使得$a=kd$。

**定义4**：假定$a, b$以及$n$是整数，且$n>0$。我们说，$a和b$**同模**于$n$当且仅当$n|(a-b)$，记为$a\equiv b \:(mod\: n)$。读作“a与b同模于n”。


**定理**：令$n$为一整数，如果$6|n$，则$3|n$。

*证明*：根据定义3，如果$6|n$，则存在一个整数$k$，使得$n=6*k$，则$n=6*k=3*(2k)$，若令$k'=2k$，则$n=3*k'$，满足定义3的定义，所以$3|n$。
...
**定理1.1**：令$a, b, c$为整数，如果$a|b$且$a|c$，则$a|(b+c)$

*证明*：根据定义3，$a|b$表明$b=a*k_1$，$a|c$表明$c=a*k_2$，所以$b+c=a*(k_1+k_2)$，所以$a|(b+c)$。证毕。

**定理1.2**：令$a, b, c$为整数，如果$a|b$且$a|c$，则$a|(b-c)$

**定理1.3**：令$a, b, c$为整数，如果$a|b$且$a|c$，则$a|(b*c)$
...
**定理1.3-1**：令$a, b, c$为整数，如果$a|b$且$a|c$，则$a^2|(b*c)$

**定理1.3-2**：令$a, b, c$为整数，如果$a|b$，则$a|(b*c)$
...
**定义**：整数$a, b$的公约数是指这样一个整数$d$，使得$d|a$且$d|b$。

**定义**：两个整数$a, b$（两者不同时为0）的最大公约数是最大的一个整数$d$，使得$d|a$且$d|b$。$a, b$的最大公约数通常记为$gcd(a, b)$或者简单记为$(a, b)$。

**定义**：如果$gcd(a, b)=1$，那么$a, b$被称为`互质`（`relatively prime`）。

**定理1.32**：令$a, n, b, r, k$为整数。如果$a=nb+r, k|a, k|b$，则$k|r$。

*证明*：因为$k|a, k|b$，由定义3，$a=ka', b=kb', r=a-nb=ka'-knb'=k(a'-nb')$，因此$k|r$。
...
**定理1.40**：对任意的整数$a, b$，且$a, b$不同时为0，则必然存在一对整数$x, y$，使得$ax+by=(a, b)$
