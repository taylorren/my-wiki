...
**定理3.14**：对任意的整数$a$和自然数$n$，在集合$\{0, 1, 2, ..., n-1\}$中一定存在唯一的一个整数$t$，使得$a\equiv t\:\:(mod\:n)$。

*证明*：

根据[“分而治之”](../divide)中`除法算法`的描述，对于题中给定的$a$和$n$，存在$a=bn+t$，而且$0\le t \lt n$。所以$t\in \{0, 1, 2, ..., n-1\}$，而且满足$a\equiv t\:\:(mod\:n)$。QED。
...
令$A=\{a_1, a_2, ..., a_n\}$，如果$i\ne j \implies a_i\not\equiv a_j\:\:(mod\:n)$。根据除法算法，$a_i=q_in+r_i, a_j=q_jn+r_j\implies r_i\ne r_j$，而$0\le r_i, r_j\le n-1$。于是$n$个$A$中的元素必然有$n$个不同的$r$，$\forall a_k\in A,\: \exists r_k\in\{0, 1, ..., n-1\}$，使得$a_k\equiv r_k\:\:(mod\:n)$。根据之前的定理，$A$构成一个完全余数集。QED。

## 线性同余

在第一章“[分而治之](../divide)”中，我们讨论了如何求解线性丢番都方程。我们现在开始着手处理模运算中的类似问题。我们的下一个目标是确定形如$ax\equiv b\:\:(mod\:n)$这样的方程有没有解。

**练习3.18**：在合适的标准完全余数集中找到下列同余方程的解。

1. $26x\equiv 14\:\:(mod\:n)$
...
1. $24x\equiv 123\:\:(mod\:213)$

*解*：这题用试错法做太累了。我们稍后来求解。

**定理3.19**：令$a, b, n \in Z, n\gt 0$。那么$ax\equiv b\:\:(mod\:n)$有解当且仅当存在$x, y\in Z\:s.t\:ax+ny=b$。

*证明*：假定$ax\equiv b\:\:(mod\:n)$有解，那么存在一个整数$z$并满足$ax-b=nz \implies ax+nz=b$，定理成立。

又，假定存在$x,y$使得$ax+ny=b$成立，那么$ax-b=n(-y)\implies n\mid(ax-b)\implies ax\equiv b\:\:(mod\:n)$。

**定理3.20**：令$a, b, n \in Z, n\gt 0$。那么等式$ax\equiv b\:\:(mod\:n)$有解当且仅当$(a,n)\mid b$。

*证明*：有定理3.19以及定理1.48后，我们可以很方便证明本定理。

我们回头看之前留下的练习：$24x\equiv 123\:\:(mod\:213)$是否有解呢？
