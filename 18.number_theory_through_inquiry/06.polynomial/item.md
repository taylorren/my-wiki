...
显然，对于素数的幂次$p^k$来说，它的因子也很容易找到：$0, p, 2p, 3p, ...$，也就是每$p$个数有一个，所以一共有$p^k/p=p^{k-1}$个。而$0, 1, 2, ..., p^k-1$一共有$p^k$个数字，所以$\phi(p^k)=p^k-p^{k-1}$。

于是我们可以证明一个很重要的引理：

**引理6.12**：如果$p$是一个素数，那么$\sum_{d\mid p^k}=p^k$。
...
将这些式子累加得到$\sum_{d\mid p^k}=p^k$。QED。
