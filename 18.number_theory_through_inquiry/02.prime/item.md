...
首先，所有的$p_i \mid n$。因为$p_i\mid (n+1)$，而根据定理2.32，两个相邻的自然数互质，既然$n+1$包含了所有$p_i$作为质因子，$n$就不能包括$p_i$为质因子。所以$n$的质因子不在$S$中。
...
所以，$n$中一定存在一个质因子$p$，s.t $p\equiv 3\:\:(mod\:4)$，但它不是$S$中的任何一个。所以，我们的假设不成立，也就证明存在无限多个形如$4k+3$的素数。QED。
